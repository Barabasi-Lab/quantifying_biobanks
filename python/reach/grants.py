...
cohort_grants['funder_country'] = cohort_grants['funder_org_countries'].apply(lambda x: x[0] if x is not np.nan else np.nan)
cohort_grants = cohort_grants.dropna(subset=['activity_code'])
...
# save to ../../data/expansion/database/cohort_reach/grants.csv
cohort_grants_data.to_csv('../../data/expansion/database/cohort_reach/grants.csv', index=False)