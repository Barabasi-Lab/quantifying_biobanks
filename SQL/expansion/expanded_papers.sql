...
CREATE OR REPLACE TABLE  `ccnr-success.cohorts.expanded_papers` AS (
...
expanded_papers_of AS (
  SELECT
    CASE
      ...
      WHEN REGEXP_CONTAINS(title.preferred, program_of) THEN REGEXP_EXTRACT_ALL(title.preferred, program_of)
      WHEN REGEXP_CONTAINS(abstract.preferred, program_of) THEN REGEXP_EXTRACT_ALL(abstract.preferred, program_of)
      WHEN REGEXP_CONTAINS(acknowledgements.preferred, program_of) THEN REGEXP_EXTRACT_ALL(acknowledgements.preferred, program_of)
